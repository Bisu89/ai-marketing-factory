...
def _build_user_message(video: Video, extra_context: str | None = None) -> str:
    ...
    # Task 04 -- see app/modules/ai/story/service.py's own identical
    # comment. Only content_idea_generation.py (the composition root
    # bridging content_strategy into this module) ever passes this; every
    # other caller (plain POST /hook-jobs) leaves it None.
    if extra_context:
        message = f"{message}\n\n{extra_context}"
    return message
...
class HookService:
    ...
    def generate(self, video_id: int, extra_context: str | None = None) -> HookJob:
        ...
        user_message = _build_user_message(video, extra_context)
